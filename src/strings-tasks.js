...
function removeLastOccurrences(str, value) {
  const array = str.split(value);
  if (array.length > 2) {
    const first = array[0];
    array.shift();
    const string = `${first}${value}${array.join('')}`;
    return string;
  }
  return array.join('');
}
...
function formatTime(minutes, seconds) {
  const m = minutes.toString(10).padStart(2, '0');
  const s = seconds.toString(10).padStart(2, '0');
  return `${m}:${s}`;
}
...
function findLongestWord(sentence) {
  ...
  const lengthArray = array.map((elem) => elem.length);
...
function extractNameFromTemplate(value) {
  ...
  return `${array[1]} ${array[2].substring(0, array[2].length - 1)}`;
}
...
function encodeToRot13(str) {
  ...
  const encrypted = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
  return str
    .split('')
    .map((letter) => {
      if (original.includes(letter)) {
        return encrypted[original.indexOf(letter)];
      }
      return letter;
    })
    .join('');
